...
foreach ($types as $t) {
    ...
    $stmt->execute([$t['id']]);
    $typeCounts[$t['id']] = (int) $stmt->fetchColumn();
}
...
renderPage('لوحة التحكم', 'dashboard', function() use (
    $types, $typeCounts, $totalBeneficiaries, $totalDistributions, $latestDist, $cardColors
) {
...
<div class="hero-header mb-4">
    <div class="row align-items-center g-0">
        <div class="col">
            ...
            <p>اليوم: <?= date('l، d F Y') ?> &nbsp;|&nbsp; إجمالي المستفيدين النشطين: <strong><?= $totalBeneficiaries ?></strong></p>
        </div>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-4 col-xl-2">
        <div class="stat-card" style="background: <?= $cardColors[$i % count($cardColors)] ?>;">
            <div class="stat-value"><?= $typeCounts[$t['id']] ?></div>
            <div class="stat-label"><?= e($t['name']) ?></div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= $d['type_name'] ? ('<span class="badge bg-primary">' . e($d['type_name']) . '</span>') : '<span class="text-muted">—</span>' ?></td>
                        <td><?= e($d['distribution_date']) ?></td>
                        <td><?= e((string)$d['item_count']) ?></td>
                        <td><?= formatAmount($d['total_cash']) ?></td>
                        <td>
                            <a href="<?= e(ADMIN_PATH) ?>/print_distribution.php?id=<?= $d['id'] ?>" target="_blank" class="btn btn-sm btn-outline-secondary">
